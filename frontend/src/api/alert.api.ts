...
export const alertApi = {
  ...
  // POST /alerts
  async createAlert(payload: AlertSaveRequest) {
    const { data } = await http.post<Envelope<SimpleOk>>('/alerts', payload)
    return data
  },
  ...
  async changeAlertStatus(alertId: number, payload: { status: string }) {
    const { data } = await http.patch<Envelope<SimpleOk>>(`/alerts/${alertId}/status`, {
