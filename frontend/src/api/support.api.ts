...
export const supportApi = {
  ...
  async getNotice(id: number) {
    const { data } = await http.get<Envelope<NoticeDetailInfo>>(`/supports/notices/${id}`)
    return data
  },

  // GET /notices
  async getNotices(params?: NoticeListRequest) {
    const { data } = await http.get<Envelope<NoticeInfo[]>>('/supports/notices', { params })
    return data
  },

  // GET /faqs
  async getFAQs(params?: FAQListRequest) {
    const { data } = await http.get<Envelope<FAQInfo[]>>('/supports/faqs', { params })
    return data
  },
}
