...
export function useChangePasswordForm() {
  ...
  function validate(): boolean {
    ...
    if (!np) errs.newPassword = '새 비밀번호를 입력해주세요.'
    else if (!minLength(np, 8)) errs.newPassword = '비밀번호는 8자 이상이어야 합니다.'
    else if (!isStrongPassword(np))
      errs.newPassword = '비밀번호는 영문/숫자/특수문자를 모두 포함해야 합니다.'

    if (!npc) errs.confirmPassword = '비밀번호 확인을 입력해주세요.'
    else if (np !== npc) errs.confirmPassword = '비밀번호가 일치하지 않습니다.'
