...
export function mapCreateAlertError(error: ApiError | null): string | null {
  if (!error) {
    return '네트워크 오류가 발생했습니다. 잠시 후 다시 시도해주세요.'
  }

  if (error.code === 'conflict' && error.target === 'alert_status') {
    return '알림은 활성 상태로만 생성할 수 있습니다.'
  }

  if (error.code === 'conflict' && error.target === 'user_alert') {
    return '전체 알림은 최대 30개까지 만들 수 있습니다. 사용하지 않는 알림을 삭제한 뒤 다시 시도해주세요.'
  }

  if (error.code === 'conflict' && error.target === 'user_active_alert') {
    return '활성 알림은 최대 5개까지 사용할 수 있습니다. 사용하지 않는 알림을 비활성화한 뒤 다시 시도해주세요.'
  }

  if (error.code === 'not_found' && error.target === 'alert_type') {
...
export function mapChangeAlertError(error: ApiError | null): string | null {
  if (!error) {
    return '네트워크 오류가 발생했습니다. 잠시 후 다시 시도해주세요.'
  }

  if (error.code === 'not_found' && error.target === 'alert') {
...
export function mapChangeAlertStatusError(error: ApiError | null): string | null {
  if (!error) {
    return '네트워크 오류가 발생했습니다. 잠시 후 다시 시도해주세요.'
  }
  ...
  if (error.code === 'conflict' && error.target === 'user_archived_alert') {
