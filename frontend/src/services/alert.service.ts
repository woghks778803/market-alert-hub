...
export async function getAlertLogs(
  payload: AlertLogListQuery
): Promise<CursorListResult<AlertLogDto>> {
...
export async function getArchivedAlerts(
  payload: ArchivedAlertListQuery
): Promise<CursorListResult<AlertDto>> {
...
export async function createAlert(payload: AlertSaveQuery) {
  const env = await alertApi.createAlert(toAlertSaveRequest(payload))
