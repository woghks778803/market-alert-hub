...
class OutboxService:
    ...
    def process_one(self, event_id: int, dispatch_fn) -> None:
        with self.uow_factory() as uow:
            ...
            if row is None:
                logger.warning("outboxs not found id=%s", event_id); return
            if row.status != OutboxStatus.SENDING:
                logger.info("skip id=%s status=%s", event_id, row.status); return

            payload = OutboxRule.parse_payload(getattr(row, "payload_json", None))
            try:
                dispatch_fn(event_type=row.event_type, channel=row.channel, payload=payload)
                uow.outboxs.mark_sent(row.id, row.attempts)
                # __exit__ commit
            except Exception as e:
                attempts = row.attempts + 1
                if attempts < OutboxRule.MAX_ATTEMPTS:
                    next_at = datetime.now(timezone.utc) + OutboxRule.compute_backoff(attempts)
