...
def list_channel_providers(
    ...
    rows = svcs.channels.list_provider_by_filter(is_active=is_active, limit=limit, offset=offset)
    return ok(rows, request_id=meta.request_id)


@router.get(
    "/{user_channel_id}",
    response_model=Envelope[ChannelSchema.UserChannelRead],
    summary="사용자 채널 상세",
    responses=OpenApi.combine(
        OpenApi.OK(Envelope[ChannelSchema.UserChannelRead]),
    ),
)
def get_channel(
